...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: pain category
      decision: workflow friction — manual calculations and parameter juggling across tools
    - topic: insight
      decision: simplified, refined UX for home brewing — existing tools are limited or overly complex
    - topic: primary persona scope
      decision: home brewer (hobbyist) — solo, small-batch
    - topic: access model
      decision: login required; flat user model, no roles
    - topic: mvp scope
      decision: create + save + view list only; no edit/delete in v1; 3 weeks after-hours
  frs_drafted: 13
...
- **Model:** Login required — account-based access (email/password, OAuth, or passwordless; specific mechanism deferred to implementation)
...
All elements present — no gaps recorded.
