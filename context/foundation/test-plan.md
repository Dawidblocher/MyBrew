...
| 2   | Ochrona autoryzacji          | Udowodnić izolację przepisów per user i że mutacje wymagają potwierdzenia własności zasobu                                 | #2, #4 | Testy integracyjne auth/API (Supabase SSR lub mock) | not started | context/changes/testing-auth-read-boundary/       |
...
- **E2E runner:** Playwright (`@playwright/test`, `playwright.config.ts`, `testDir: "./tests"`, `webServer: npm run dev`). Projekt `setup` (`tests/auth.setup.ts`) loguje się przez UI i zapisuje `storageState` dla projektów zalogowanych (`chromium`/`firefox`/`webkit`). Osobny projekt `guest` (Desktop Chrome, bez `storageState`, bez zależności od `setup`) pokrywa granicę sesji gościa. Brak drugiego konta E2E — realny cross-user/RLS odroczony do zmiany Ryzyka #2.
- **CI:** GitHub Actions — lint + build + testy (`npm run test:run`) na każdy push/PR do master (`ci.yml`, sekrety `SUPABASE_*`). Workflow `.github/workflows/playwright.yml` uruchamia wyłącznie `npx playwright test --project=guest` z `SUPABASE_URL`/`SUPABASE_KEY`; nie przekazuje `E2E_USERNAME`/`E2E_PASSWORD` i nie uruchamia projektów zalogowanych.
- **Profil bazy testów:** Vitest w `src/lib/` (silnik calc, seam zapisu, izolacja zapytań cross-user, polityka `isProtectedPath`). Playwright: `tests/auth-read-boundary.spec.ts` (gość) + setup zalogowany lokalnie. Zero testów komponentów React / stron Astro poza E2E gościa.
...
Wzorce z `context/changes/testing-auth-read-boundary/` (change `testing-auth-read-boundary`) — **tylko Ryzyko #4 (odczyt)**. Status Fazy 2 w §3 pozostaje `not started`, bo Ryzyko #2 (IDOR na mutacjach) jest nadal otwarte.
...
- **CI:** `.github/workflows/playwright.yml` przekazuje wyłącznie `SUPABASE_URL`/`SUPABASE_KEY` i uruchamia `--project=guest`.

**Odroczone do osobnej zmiany IDOR (Ryzyko #2):** realny cross-user z dwoma kontami E2E / JWT (dowód RLS), testy PUT/DELETE cudzego zasobu → 404.
